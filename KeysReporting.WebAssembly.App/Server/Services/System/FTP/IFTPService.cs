...
namespace KeysReporting.WebAssembly.App.Server.Services.System.FTP
{
    ...
    public class FTPService : IFTPService
    {
        ...
        private FTPService(CallDispositionContext callDispositionContext, IConfiguration configuration, ILogger<FTPService> logger)
        {
                ...
                if (file.Filename.Contains("KeysAgentDispostions") && DateTime.Parse(file.LastModifiedTimeStr) >= DateTime.Today.AddDays(-7))
                {
                    try
                    {
                            ...
                            && !string.IsNullOrEmpty(x.ProjectName) && !string.IsNullOrEmpty(x.ClientName)))
                        {
                            ...
                            if (!_callDispositionContext.TermCodes.Where(x => x.TermCode1.ToLower() == record.TermCode.ToLower()).Any())
                                continue;
                            ...
                            ftpFile.CallDispositions.Add(new CallDisposition
                            {
                                ...
                                FkTermCodeNavigation = await _callDispositionContext.TermCodes.Where(x => x.TermCode1.ToLower() == record.TermCode.ToLower()).FirstOrDefaultAsync(),
